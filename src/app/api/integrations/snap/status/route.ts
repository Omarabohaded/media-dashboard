...
export async function GET(request: NextRequest) {
  ...
  const client = await getRequiredClientById(access.clientId), connection = await getSnapConnection(client.id), config = getSnapConfig();
...
export async function DELETE(request: NextRequest) { const access = await requireClientIntegrationAccess(request.nextUrl.searchParams.get("clientId")); if (access.response) return access.response; const client = await getRequiredClientById(access.clientId); await clearSnapConnection(client.id); return NextResponse.json({ ok: true }); }
