...
export async function POST(request: NextRequest) {
  ...
  const body = await request.json() as { adAccountId?: string }; const accounts = await withSnapAccess(client.id, fetchSnapAdAccounts), account = accounts.find((item) => item.adAccountId === body.adAccountId);
